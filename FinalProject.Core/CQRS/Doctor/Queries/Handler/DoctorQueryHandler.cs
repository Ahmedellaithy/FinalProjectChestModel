...
public class DoctorQueryHandler : ExtraResponseHandler,
                                  ...
                                  IRequestHandler<GetDoctorPaginatedQuery,PaginationResponseOutput<GetDoctorPaginatedResponse>>
                                    ...
                                    //IRequestHandler<GetDoctorListQuery,ICollection<GetDoctorListResponse>>,
                                 
{
    ...
    public async Task<ExtraResponseOutput<GetDoctorByIdResponse>> Handle(GetDoctorByIdQuery request, CancellationToken cancellationToken)
    {
        ...
        if (doctor == null) return BadRequest<GetDoctorByIdResponse>("Doctor not found");
        
        var mappedDoctor = _mapper.Map<GetDoctorByIdResponse>(doctor);
        return Success(mappedDoctor);
    }

    // public async Task<ICollection<GetDoctorListResponse>> Handle(GetDoctorListQuery request, CancellationToken cancellationToken)
    // {
    //     var paginatedDoctors = await _service.GetAllDoctorsPaginatedAsync(1, 10);
    //     var total = await _service.GetDoctorsCountAsync();
    //     return 
    // }
